...
namespace LogicalCube
{
    ...
    class Solution
    {
        ...
        private void SolveWhiteCorners()
        {
            if( debugToConsole )
            {
                Console.WriteLine(AnsiColor.Convert($"DEBUG: cube at start of SolveWhiteCorners():", "00FF00"));
                _cube.WriteColoredCube();
            }
            // Flip the cube over
            ...
            if (debugToConsole)
            {
                Console.WriteLine(AnsiColor.Convert($"DEBUG: After x2:", "00FF00"));
                _cube.WriteColoredCube();
            }
            ...
            foreach (string cornerPiece in pieceDestinations)
            {
                ...
                if (debugToConsole)
                {
                    Console.WriteLine(AnsiColor.Convert($"DEBUG: After solving {cornerPiece}:", "00FF00"));
                    _cube.WriteColoredCube();
                }
            }
        }
